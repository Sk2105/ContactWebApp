...
public class ContactService {
    ...
    public void saveContactImage(String id, MultipartFile image) {
        try (InputStream in = image.getInputStream()) {
            ...
            File targetLocation = new File(imagePath, fileName);
            if(!targetLocation.exists()) {
                targetLocation.mkdir();
            }
            Files.copy(in, targetLocation.toPath(), StandardCopyOption.REPLACE_EXISTING);

            contact.setImageUrl("http://localhost:8080/images/" + fileName);
